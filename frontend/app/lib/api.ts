...
export const uploadDocument = async (file: File) => {
    ...
    const response = await fetch(`${BASE_URL}/api/ingest`, { method: 'POST', body: formData });
    if (!response.ok) {
...
export const getDssRecommendations = async (claimId: number) => {
  ...
  if (!response.ok) throw new Error('Failed to fetch DSS recommendations');
  return response.json();
};
...
export const getAssets = async (lat: number, lon: number) => {
    const queryParams = new URLSearchParams({ lat: lat.toString(), lon: lon.toString() });
    const response = await fetch(`${BASE_URL}/api/assets?${queryParams.toString()}`);
